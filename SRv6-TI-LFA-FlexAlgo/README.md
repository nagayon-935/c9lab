。ぎちgのれ# SRv6 TI-LFA + Flex-Algo ラボ
...
> **Note:** XRd 25.2.2 では `advertise-delay static <value>` は無効。`advertise-delay <value>` が正しい構文。
...
> **Note:** IS-IS グローバル address-family レベルの `fast-reroute per-prefix ti-lfa` は XRd 25.2.2 では無効。インターフェースレベルのみ有効。

### L3VPN
- BGP AS: 65100
- VRF: `CUST-A` (RT-01 / RT-06)
- PE間: iBGP VPNv4 直接ピア (RT-01 ↔ RT-06)
- SRv6 encapsulation: `H.Encaps.Red`、locator `ALGO0` を使用
...
cafe:80:1:e000::    uA (PSP/USD)   [Gi0/0/0/1]:128:P    isis-1
...
```
...
RT-01# show cef vrf CUST-A 100.0.6.0/24 detail
  SRv6 H.Encaps.Red SID-list {cafe:0:6:e004::}   ← RT-06 の uDT4 SID
  via GigabitEthernet0/0/0/1, fe80::...(RT-02)   ← 上パス経由
```

### 8. TI-LFA フェイルオーバーテスト
...
# CEF (SRv6 エンキャプ確認)
show cef vrf CUST-A 100.0.6.0/24 detail
```
...
- **`advertise-delay static` コマンド不存在 (XRd 25.2.2)**: `advertise-delay <value>` で指定する。
- **IS-IS global AF の `fast-reroute per-prefix ti-lfa` は無効**: インターフェースレベルの `address-family` で設定する。
- **L3VPN は ALGO0 ロケータのみ使用**: Flex-Algo 128 パス上に L3VPN を転送させる場合は BGP VRF の `locator ALGO128` への変更が必要。
- **SSH レート制限**: XRd への SSH 接続を短時間に繰り返すとレート制限が発動する。
